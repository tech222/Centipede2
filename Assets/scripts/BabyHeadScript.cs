...
public class BabyHeadScript : MonoBehaviour {
    
    //public GameObject BabySegPrefab;                      // baby segment prefab object
    public float speed = 12.0f;                             // speed for baby
    GameObject target1;                              // target for baby to baby move toward
    GameObject target2;                              // alternate target for baby to move toward
    ...
    private int initSegments = 1;                           // initial number of segments
    ...
    void Start () {
        ...
        // zero out all elements in baby positions array
        for (int bb = 0; bb < positions.Length; bb++) 
        {
            positions [bb] = transform.position;    
        }
        
        //seg[0] = GameObject.Find ("BabySegPrefab");
        seg[0] = gameObject.transform.parent.GetChild(1).gameObject;
        segSprite[0] = seg[0].GetComponent<SpriteRenderer>();
        segSprite[0].sortingOrder = -1;
        
        Debug.Log ("babyseg segment name: " + seg[0].name);
        
        if (GameObject.Find ("playerHomePrefab")!= null)
        {
            target1 = GameObject.Find ("playerHomePrefab");
        }
        else
        {
            target1 = null;
        ...
        if (GameObject.Find ("playerHome2")!= null)
        {
            target2= GameObject.Find ("playerHome2");
        }
        else
        {
            target2 = null;
    ...
    {
        wallcheckOff = true;
        seg[0].GetComponent<Collider2D>().enabled = false;
